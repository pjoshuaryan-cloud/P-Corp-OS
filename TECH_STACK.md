...
**Status:** Skeleton — UI shell platform, backend language, IPC mechanism, and process lifecycle are decided; database, sync, auth, and packaging are still open.
...
- Packaging: how the Python backend gets bundled inside the SwiftUI app so `SMAppService` can register it as a login item — this is now more concrete than before (SMAppService requires the helper to ship inside the main app's bundle structure), but the actual bundling mechanics (embedding a Python runtime, dependency freezing, etc.) aren't solved yet.
...
Packaging (how the Python backend actually ships inside the SwiftUI app bundle so `SMAppService` can manage it) is the most concrete remaining question — everything else (database, sync, auth) can be designed somewhat independently, but packaging is a prerequisite for the process-lifecycle decision to be buildable at all.
