...
Auth: full per-device auth (Keychain/Secure Enclave) or cloud/device sync are both genuinely premature right now — both need a second device to mean anything, and there's only one. Better candidates: forgetting/versioning for memory records, one of the 8 remaining placeholder nav sections, or `SMAppService` packaging (deferred once already as a bigger 3-stage undertaking) — otherwise keep going in order of importance per Joshua's standing instruction, unless he names something else.
