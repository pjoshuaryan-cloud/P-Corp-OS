...
**Unbuilt-section placeholder — designed, not bare (2026-07-24).** With War Room and Settings now genuinely polished, the other 9 nav sections showing a bare icon and gray "Not built yet" text had started to look like an oversight rather than an honest placeholder. Redesigned `SectionPlaceholderView` (`ContentView.swift`) to match the shell's actual quality bar while staying just as honest: the section's icon sits in a glass-material badge, its title and subtitle both show (subtitle gives a sense of what the section is for), a small **"STANDBY"** status pill (muted dot + tracked label, echoing the "Active" pill on Mission Status but in a deliberately inactive state) replaces plain text as the primary signal, with the explicit "Not built yet — this is an honest placeholder, not a preview" line kept as a smaller caption underneath so the honesty is still front and center, not buried. A brief fade+scale on appear, matching the rest of the shell's entrance treatment.

**Keyboard shortcuts (added 2026-07-24):** Cmd+1 through Cmd+9 jump directly to the first nine sidebar sections by position; Cmd+, jumps to Settings (standard macOS convention for a preferences pane). Genuine, ordinary SwiftUI functionality — not a visual/rendering risk the way the logo and particle work were. Fits the "command-center" framing of the War Room concept: a keyboard-driven power-user layer underneath the visual one.
