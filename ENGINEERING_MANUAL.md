...
**Structure as of the first real prototype (2026-07-23):** `desktop/` holds the SwiftUI app as a Swift Package Manager executable target (`desktop/Package.swift`, sources under `desktop/Sources/PCorpOS/`) — not a full Xcode project, since only command-line developer tools are installed on the build machine, not full Xcode. SPM + SwiftUI's native `App` lifecycle builds and runs (`swift build` / `swift run`) without needing Xcode at all; a full `.xcodeproj` can be introduced later if/when Xcode-specific tooling (asset catalogs, entitlements for `SMAppService`, notarization) is actually needed.

**Backend added 2026-07-24:** `backend/` holds the Python service, managed with `uv` (`backend/pyproject.toml`, `backend/uv.lock` committed for reproducibility, `backend/.venv/` gitignored). `uv` manages its own Python 3.12 interpreter independent of this machine's old system Python (3.9.6), so no compatibility concerns there. FastAPI app under `backend/app/`, run with `uv run python -m app.main`. First slice is deliberately echo-only (see `TECH_STACK.md`/`ROADMAP.md`) — proves the IPC contract, not real Frank intelligence yet.

**Known platform quirk — raw SPM executables don't get normal foreground/key-window status automatically (found 2026-07-24).** Keyboard shortcuts (Cmd+1-9, Cmd+,) were installed correctly (a local `NSEvent` monitor) but silently received zero key events. Diagnosed by logging `NSApp.activationPolicy()`/`isActive`/`keyWindow` directly: a plain `swift build`/`swift run`-launched executable — not a proper Xcode-built `.app` — came back `activationPolicy = .prohibited`, `isActive = false`, `keyWindow = false` on launch, even though mouse/click events already worked fine (they route differently and don't require "key" status). Fix: explicitly call `NSApp.setActivationPolicy(.regular)`, `NSApp.activate(ignoringOtherApps: true)`, and `window.makeKeyAndOrderFront(nil)` shortly after launch (see `ContentView.swift`'s `onAppear`). **This will likely resurface for anything else that needs real key-window status** (text field auto-focus on launch, modal dialogs, etc.) until this app is packaged as a proper `.app` bundle — worth retesting once that happens, since a real bundle may not need the workaround at all.
