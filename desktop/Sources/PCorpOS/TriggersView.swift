...
/// The "Triggers" nav section (2026-08-21) -- the Proactive Triggers
/// Layer's UI, requested right after the backend/email path was built and
/// verified end to end. Same GET-and-render pattern as AutomationsView/
/// TradingDivisionView, but the first section screen with a real UI-driven
/// write (the per-rule enabled toggle) -- everywhere else so far, Frank is
/// the only writer; this is plain settings-style state with no reason to
/// route through a conversation.
///
/// Placed in the sidebar's INTELLIGENCE group right after Automations
/// (`Sidebar.swift`'s navGroups) -- architecturally the closest sibling:
/// Automations is event-driven ("whenever X happens"), Triggers is
/// scheduled ("check every day whether X is still true"). Appended to
/// `PlaceholderData.navItems` after Automations rather than inserted
/// earlier in the array, since `ContentView`'s Cmd+1-9 shortcuts index by
/// position — this lands at position 10, past Cmd+9's reach, same as
/// Calendar/Settings already were, so nothing existing breaks.
