...
/// Joshx project detail (2026-08-31) -- the first UI-driven write in
/// Joshx (every prior status/payment change happened only via a Frank
/// chat message). Reached by tapping a row in JoshxView's PROJECTS
/// section, presented as a popover -- same mechanism as TheBriefPopover/
/// ConversationListPopover, desktop's established modal convention (iOS
/// uses `.sheet` instead; see JoshxProjectDetailSheet.swift).
///
/// Client info is looked up client-side against `client.dashboard?.clients`
/// (already fetched for the CLIENTS section on the same screen) rather
/// than a new backend field -- `projects.client_name` is plain text, no
/// foreign key to a client row (a project can exist with no matching
/// client record at all), so this is a best-effort name match, not a
/// guaranteed join.
struct JoshxProjectDetailPopover: View {
