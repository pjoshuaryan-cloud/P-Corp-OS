...
private struct ChatBubble: View {
    ...
    // updated 2026-09-11 for iPhone independence). Desktop still tries
    // local disk first (ProjectPaths.repoRoot, same shortcut already used
    // for auth_token/Knowledge docs) -- fast, no network round trip, and
    // still correct whenever the Mac's own backend is what generated it.
    // But that's no longer guaranteed once documents can be generated by
    // *either* backend: falls back to the same authenticated network
    // fetch iOS already uses (BackendClient.documentURL) when the local
    // file doesn't exist, so a document Frank generated via the cloud
    // instance still opens correctly here. NSWorkspace.shared.open hands
    // either path to the user's default PDF viewer (Preview), which
    // already provides full view + save (Cmd+S, drag out, Reveal in
    // Finder) natively -- no custom save-panel code needed either way.
    @ViewBuilder
    private var generatedDocumentRows: some View {
        ForEach(message.generatedDocuments) { document in
            GeneratedDocumentRow(document: document) {
                let localURL = ProjectPaths.repoRoot.appendingPathComponent("backend/data/generated_documents/\(document.filename)")
