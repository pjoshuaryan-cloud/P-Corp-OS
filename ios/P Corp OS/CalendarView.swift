...
/// Seventh and final deferred sidebar section (2026-08-14). Same UI as
/// desktop's own CalendarView.swift -- day-grouped event list, read-only --
/// but backed by iOS's own SystemCalendar.swift (EventKit against the
/// phone's own Calendar app), not a port of desktop's AppleScript-based
/// one. See SystemCalendar.swift's docstring for why that data source is
/// genuinely different, not just a different mechanism to the same one.
